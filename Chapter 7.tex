...
\begin{enumerate}
	...
	\[
		...
		\end{aligned}
	\]
	\item Let's clarify a bit of terminology before we begin. We are looking at just the fermions of the left moving and right moving sides of the heterotic string theory. On the left-hand (supersymmetric) side, in the light-cone gauge these form an $\widehat{O(8)}$ current algebra at \emph{level 1}. On the right-hand side the form a $\widehat{O(32)}$ current algebra at level 1 again (\textbf{why must we always have level 1? Ask Xi.}). In both cases the left and right fermions share the same boundary conditions.
	
	The characters of $\widehat{O(N)}_1$ for $N$ even correspond to the integrable representations labeled by $\O, V, S, C$ corresponding to the trivial, vector, spinor, and conjugate spinor. Our partition functions in question can be constructed from integer linear combination of products of exactly one $\widehat{O(8)}_1$ and one $\widehat{O(32)}_1$ character. This gives sixteen possible terms.
	
	The normalization of the identity contribution $\chi_{8, O} \chi^*_{32, O}$ to $1$ reduces this to $15$. 
	% The characters of $\widehat{O(N)}_1$ are expressible in terms of $\frac12 \left[\frac{\theta\twist ab(\tau)}{\eta}\right]^{N/2}$ and we will write this in shorthand as $\frac12 Z \twist ab^{N/2}$ as in Polchinski. Before imposing \emph{any} constraints the most general partition function I can write must take the form as a sum of products of one character from the left and one character from the right.
% 	\[
% 		\sum_{a, b, a', b'} \alpha_{a,b,a',b'} Z \twist ab^4 Z^* \twist ab^{16}
% 	\]
% 	This gives $16$ coefficients $\alpha$. The coefficients are contstrained to be integers.
	
	For the next part of this problem, it is important to know that the only integrable representation of $\widehat{E_8}_1$ is the identity which we will denote by $\chi_E= \frac12 \sum_{ab} \theta^8 \twist ab/\eta^8$. This immediately shows that for the case $G = E_8 \times E_8$ the only possibility is the heterotic E theory.
	
	For $G = E_8 \times O(16)$ we again have four characters: $\chi_{E} \chi_{16, \O}, \chi_E \chi_{16, V}, \chi_E \chi_{16, S}, \chi_E \chi_{16, C}$. 
	
	Finally, the hardest case: $O(16) \times O(16)$
