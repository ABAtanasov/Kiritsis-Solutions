...
% 	\textbf{FINISH THIS}
  ...
  \begin{center}
  	\includegraphics[scale=0.16]{"Drawings/Plane to Cylinder"}
  \end{center}
...
% 	\end{itemize}
	...
	\textbf{Interpret this in terms of image charges}
	  ...
	  \begin{center}
	  	\includegraphics[scale=0.12]{"Drawings/DN Plane"}
	  \end{center}
    ...
    \begin{center}
    	\includegraphics[scale=0.12]{"Drawings/D Disk"}
    \end{center}
	
	\item Using the doubling trick we have $\bar \psi (\bar z) = \psi(z^*)$. So $z_i = $ We can compute the correlator by Wick contraction:
	\[
		\braket{ \prod_{i=1}^m \psi(z_i) \, \prod_{j=1}^{2n-m} \bar \psi(\bar z_j)} = \braket{ \prod_{i=1}^n \psi(w_i) } = \frac{1}{2^n n!}\sum_{\pi \in S_{2n}} \mathrm{sgn}(\pi) \prod_{i=1}^n \frac{1}{w_{\pi(2i-1)} - w_{\pi(2i)}}  = \mathrm{Pf}\Big[\frac{1}{w_i - w_j}\Big]
	\]
	where $w_i = z_i$ for $z = 1 \dots m$ and $w_{i+m} = z_i^*$ for $z = 1 \to 2n-m$
    ...
    \begin{center}
    	\includegraphics[scale=0.1]{"Drawings/DN Strip"}
    \end{center}
